...
> ⚠️ **This document describes one specific deployment** — mine, an Incus
> container on a Linux host on the same LAN as the Tempest hub. The scripts in
> this directory are written for that environment. They probably work elsewhere,
> but they aren't meant as a general-purpose installer. If you're rolling your
> own setup, treat this as a reference implementation, not a recipe.
...
- **HTTP :80 reachable** from wherever you want to look at the dashboard. If
  you'd rather use a different port, edit `WX01_HTTP_PORT` in `wx01.service`
  and drop the `CAP_NET_BIND_SERVICE` line.
